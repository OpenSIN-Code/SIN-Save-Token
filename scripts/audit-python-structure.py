        ...
        if terminated:
            findings.append(
                Finding(
                    ...
                    message="Statement appears after unconditional return/raise/break/continue.",
                    scope=scope,
                )
            )
            ...
            if previous is not None:
                findings.append(
                    Finding(
                        ...
                        message=f"{statement.name!r} already defined at line {getattr(previous, 'lineno', 0)}.",
                        scope=scope,
                    )
                )
            else:
        ...
        if not harmless and fingerprint == previous_fingerprint:
            findings.append(
                Finding(
                    path=str(path),
                    line=line,
                    code="DUPLICATE_ADJACENT_STATEMENT",
                    message="Adjacent statement is structurally identical to its predecessor.",
                    scope=scope,
                )
            )
